...
export const createAndBroadcastCertificate = async (rpcEndpoint: string, wallet: any) => {
  ...
  saveCertificate(wallet.accounts[0].address, certificate);
...
export const loadCertificates = (walletId: string) => {
  ...
  return certs.filter(cert => cert.walletId === walletId);
}
...
export const saveActiveSerial = (walletId, idx: number) => {
...
const decodeCertificate = (cypher: string): TLSCertificate => {
  const secret = encryptionKey();
